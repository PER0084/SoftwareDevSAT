...
class StudyBuddyApp(CTk):
    ...
    def _shake_animation(self):
        """Shakes the main content frame on error."""
        def shake(step=0, direction=1):
            if step < 6:
                offset = 5 * direction
                self.main_content_frame.place_configure(relx=0.5, rely=0.5, x=offset, anchor="center")
                self.after(50, shake, step + 1, -direction)
            else:
                self.main_content_frame.place_configure(relx=0.5, rely=0.5, x=0, anchor="center")
        shake()
